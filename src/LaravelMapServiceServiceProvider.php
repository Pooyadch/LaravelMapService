<?php

namespace Pooyadch\LaravelMapService;

use Illuminate\Support\ServiceProvider;

class LaravelMapServiceServiceProvider extends ServiceProvider
{
    /**
     * Perform post-registration booting of services.
     *
     * @return void
     */
    public function boot()
    {
        // $this->loadTranslationsFrom(__DIR__.'/../resources/lang', 'pooyadch');
        // $this->loadViewsFrom(__DIR__.'/../resources/views', 'pooyadch');
        // $this->loadMigrationsFrom(__DIR__.'/../database/migrations');
         $this->loadRoutesFrom(__DIR__.'/routes.php');

        // Publishing is only necessary when using the CLI.
        if ($this->app->runningInConsole()) {

            // Publishing the configuration file.
            $this->publishes([
                __DIR__.'/../config/laravelmapservice.php' => config_path('laravelmapservice.php'),
            ], 'laravelmapservice.config');

            // Publishing the views.
            /*$this->publishes([
                __DIR__.'/../resources/views' => base_path('resources/views/vendor/pooyadch'),
            ], 'laravelmapservice.views');*/

            // Publishing assets.
            /*$this->publishes([
                __DIR__.'/../resources/assets' => public_path('vendor/pooyadch'),
            ], 'laravelmapservice.views');*/

            // Publishing the translation files.
            /*$this->publishes([
                __DIR__.'/../resources/lang' => resource_path('lang/vendor/pooyadch'),
            ], 'laravelmapservice.views');*/

            // Registering package commands.
            // $this->commands([]);
        }
    }

    /**
     * Register any package services.
     *
     * @return void
     */
    public function register()
    {
        $this->mergeConfigFrom(__DIR__.'/../config/laravelmapservice.php', 'laravelmapservice');

        // Register the service the package provides.
        $this->app->singleton('laravelmapservice', function ($app) {
            return new LaravelMapService;
        });
    }

    /**
     * Get the services provided by the provider.
     *
     * @return array
     */
    public function provides()
    {
        return ['laravelmapservice'];
    }
}